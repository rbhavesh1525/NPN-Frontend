  ...
  const handleSegmentation = async () => {
    ...
    try {
      ...
      const formattedResults = {
        totalCustomers: Object.values(result.clusters).reduce((sum, cluster) => sum + cluster.inserted_count, 0),
              ...
              {Object.entries(segmentationResults.clusters).map(([segmentName, data]) => (
                <div key={segmentName} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    <strong>{data.inserted_count}</strong> customers assigned to this segment
                  </p>
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2 mt-2">
                    <div
                      className="bg-blue-600 dark:bg-blue-400 h-2 rounded-full"
                      style={{
                        width: `${(data.inserted_count / segmentationResults.totalCustomers) * 100}%`
                      }}
                    ></div>
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {((data.inserted_count / segmentationResults.totalCustomers) * 100).toFixed(1)}% of total
                  </p>
                </div>
              ))}
            </div>
